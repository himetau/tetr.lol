...
function shapedReward(
  p: { linesCleared: number; spin: string; type: PieceType; after: Board },
  hadReadySlot: boolean,
): number {
  let r = clearReward(
    { linesCleared: p.linesCleared, spin: p.spin as "none" | "mini" | "full" },
    p.type,
    true,
  );
  if (!findLstSite(p.after)) {
    r += LOOP_DEATH_TOLL;
  }
  if (p.type === "T" && p.spin !== "full") {
    r += hadReadySlot ? -320 : WASTED_T_TOLL;
  }
  if (p.type === "I") {
    r += I_USE_TOLL;
  }
  if (breaksB2b(p.linesCleared, p.spin)) {
    r += B2B_BREAK_TOLL;
  }
  return r;
}
...
for (let g = 0; g < GAMES; g++) {
  ...
  for (let step = 0; step < MAX_PIECES; step++) {
    ...
    const add = (piece: PieceType, queueAfter: PieceType[], holdAfter: PieceType | null) => {
      for (const p of enumeratePlacements(board, piece)) {
        ...
        cands.push({
          after: p.after,
          r,
          s: r + evaluateBoard(p.after, true).score,
          queueAfter,
          holdAfter,
        });
      }
    ...
    if (holdPiece && holdPiece !== active) {
      add(holdPiece, hold ? preview : preview.slice(1), active);
    }
    if (cands.length === 0) {
      break;
    }
    ...
    if (deadStreak >= 6 || board.maxHeight() >= 16) {
      break;
    }
  }
  ...
  for (let t = rewards.length - 1; t >= 0; t--) {
    returns[t] = t === rewards.length - 1 ? heur[t] : rewards[t + 1] + GAMMA * returns[t + 1];
  }
  ...
  if ((g + 1) % 50 === 0) {
    console.error(`game ${g + 1}/${GAMES} - ${samples.length} samples`);
  }
}
...
for (const s of samples) {
  for (let i = 0; i < D; i++) {
    std[i] += (s.f[i] - mean[i]) ** 2;
  }
}
for (let i = 0; i < D; i++) {
  std[i] = Math.max(1e-6, Math.sqrt(std[i] / samples.length));
}
const yMean = samples.reduce((a, s) => a + s.y, 0) / samples.length;
const yStd = Math.max(
  1e-6,
  Math.sqrt(samples.reduce((a, s) => a + (s.y - yMean) ** 2, 0) / samples.length),
);
...
const w1 = Array.from({ length: HIDDEN }, () =>
  Array.from({ length: D }, () => (rng() * 2 - 1) * Math.sqrt(2 / D)),
);
...
for (let ep = 0; ep < EPOCHS; ep++) {
  ...
  if ((ep + 1) % 10 === 0) {
    console.error(`epoch ${ep + 1}: val MSE ${v.toFixed(4)} (var=1)`);
  }
}
