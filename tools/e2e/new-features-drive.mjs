...
if (ended) {
  ...
  ok(
    "quick: results overlay has the run",
    /m\b/.test(res) && res.includes("pieces"),
    res.slice(0, 60),
  );
}
...
await page.evaluate(() => {
  const grades = (best, good, inacc, mist) => ({
    best,
    good,
    inaccuracy: inacc,
    mistake: mist,
...
await page.evaluate(() => {
  localStorage.removeItem("lst-trainer-stats-v1");
});
await page.reload();
await page.waitForSelector("canvas", { timeout: 10000 });
for (let i = 0; i < 8; i++) {
...
const sess = await page.evaluate(
  () => JSON.parse(localStorage.getItem("lst-trainer-stats-v1")).sessions,
);
ok(
  "drill: session recorded on leave",
  sess.length === 1 && sess[0].mode === "lst",
  JSON.stringify(sess.map((s) => s.mode)),
);
