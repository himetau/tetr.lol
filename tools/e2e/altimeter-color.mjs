...
await page.evaluate(async () => {
  ...
  document.body.style.cssText = 'background:#181825;display:flex;flex-direction:column;gap:14px;padding:20px';
  window.__alts = [];
  ...
  const tick = () => {
    for (const { a, alt } of window.__alts) a.frame({ altitude: alt, climbRank: 3, climbProgress: 6, climbMultiplier: () => 0.75 }, 16);
