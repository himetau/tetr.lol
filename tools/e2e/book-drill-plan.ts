...
for (let y = 0; y < 26; y++) {
  for (let x = 0; x < 10; x++) {
    if ((rawStart.rows[y] >>> x) & 1) {
      mirStart.rows[y] |= 1 << (9 - x);
    }
  }
}
...
interface OpenerPlan {
  actions: { piece: PieceType; cells: Cells }[];
  tsd: Cells;
}
...
function solveOpener(target: OpenerTarget, queue: PieceType[]): OpenerPlan | null {
  ...
  if (cleared.clearLines().length < 2 || !startKeys.has(cleared.key())) {
    return null;
  }

  const build = (Object.entries(target.pieces) as [PieceType, Cells][]).filter(
    ([pc]) => pc !== "T",
  );
  ...
  const dfs = (
    ...
    if (tryPiece(active, hold)) {
      return true;
    }
    if (hold && hold !== active && tryPiece(hold, active)) {
      return true;
    }
    // park in hold
    if (hold === null && dfs(board, rem, qi + 1, active)) {
      return true;
    }
    return false;
  };
  if (!dfs(new Board(), build, 0, null)) {
    return null;
  }
...
if (!plan) {
  throw new Error("no seed/target combo found");
}
...
const doPlacement = (piece: PieceType, cells: Cells, desc: string, expect?: string) => {
  ...
  if (sim.active!.type !== piece) {
    ...
    if (sim.active!.type !== piece) {
      throw new Error(
        `sim desync at "${desc}": want ${piece}, active ${sim.active!.type}, hold ${sim.hold}`,
      );
    }
  }
...
for (const a of plan.actions) {
  doPlacement(a.piece, a.cells, `opener ${a.piece}`, "Book ·");
}
...
while (placed < 6 && guard++ < 20) {
  ...
  if (!adv.onBook) {
    throw new Error(`sim off book at bag2 piece ${placed}`);
  }
  if (!adv.sustainable) {
    steps.push({
      keys: [],
      desc: `bag2 queue unsustainable at piece ${placed} - plan ends here`,
      expect: "sustained",
    });
    break;
  }
  if (adv.moves.length === 0) {
    if (!adv.holdIsBook) {
      throw new Error(`bag2 stuck at piece ${placed}: no moves, no hold`);
    }
  ...
  if (!deviated) {
    ...
    steps.push({
      ...
      desc: `bag2 deviation: ${stray.type} far right (expect book hint)`,
      expect: "Book",
    });
...
if (sim.active!.type === "T" || sim.hold === "T") {
  const tsds = enumeratePlacements(sim.board, "T").filter(
    (p) => p.spin === "full" && p.linesCleared >= 2,
  );
  const chaining = tsds.find((p) => bag3Starts.has(p.after.key()));
  console.error(
    `bag2 TSD options: ${tsds.length}, chaining into bag-3 book: ${chaining ? "YES" : "NO"}`,
  );
  const tsd = chaining ?? tsds[0];
  if (tsd) {
    doPlacement("T", tsd.cells as Cells, "bag2 TSD", "TSD");
  }
}
// post-TSD: is the board a live bag-3 book state?
{
  ...
  const adv = bookAdvice(sim.board, queue, sim.hold);
  console.error(
    `post-TSD book state: onBook=${adv.onBook} sustainable=${adv.sustainable} solutions=${adv.solutions.slice(0, 3).join(" ; ")}`,
  );
}
