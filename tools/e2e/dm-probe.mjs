...
const info = await page.evaluate(async () => {
  ...
  const s = 'BLOCKY ag08@';
  c.font = '40px "Departure Mono"'; const wDM = c.measureText(s).width;
  c.font = '40px monospace'; const wMono = c.measureText(s).width;
  return { check: document.fonts.check('40px "Departure Mono"'), wDM, wMono, differ: Math.abs(wDM-wMono) > 0.5 };
});
