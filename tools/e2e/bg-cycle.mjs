...
await page.addStyleTag({ content: '.bg-dim{opacity:.12 !important} body::before{display:none !important} #app{opacity:.0 !important}' });
...
for (let i = 0; i < 10; i++) {
  await page.evaluate(async () => { const m = await import('/src/ui/background.ts'); m.nextBackground(); });
