...
results.push({ i: 'setup', desc: 'session strip shows 4-wide phase', expect: '4-wide combo', pass: phase.includes('4-wide combo'), chip: '', toast: '', dock: phase.slice(0, 120) });
...
for (let i = 0; i < plan.steps.length; i++) {
  ...
  results.push({ i, desc: step.desc, expect: step.expect ?? null, pass, chip: state.chip, toast: state.toast, dock: state.dock.slice(0, 140), comboTag: state.comboTag, bookCards: state.bookCards });
...
results.push({ i: 'patterns', desc: `patterns page has 4-Wide section (${patterns.cards} diagrams total)`, expect: '4-Wide', pass: patterns.has4w, chip: '', toast: '', dock: '' });
await page.screenshot({ path: `${shotDir}/${String(shot++).padStart(2, '0')}-patterns.png`, fullPage: false });
