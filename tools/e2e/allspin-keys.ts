...
function orientKeys(rot: Rot): string[] {
  return rot === 1 ? ["KeyX"] : rot === 3 ? ["KeyZ"] : rot === 2 ? ["KeyA"] : [];
}
function shiftKeys(x: number): string[] {
  const n = x - 4;
  return Array.from({ length: Math.abs(n) }, () => (n > 0 ? "ArrowRight" : "ArrowLeft"));
}
function rotKey(dir: 1 | -1 | 2): string {
  return dir === 1 ? "KeyX" : dir === -1 ? "KeyZ" : "KeyA";
}
...
for (const seed of seeds) {
  ...
  let found = null;
  outer: for (let preRot = 0 as Rot; preRot < 4; preRot = (preRot + 1) as Rot) {
  ...
  const keys = [
    ...orientKeys(found.preRot),
    ...shiftKeys(found.preX),
    "ArrowDown",
    rotKey(found.dir),
    "Space",
  ];
