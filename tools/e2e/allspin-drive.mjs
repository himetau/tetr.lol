...
for (const p of plan) {
  ...
  for (const k of p.keys) { await page.keyboard.press(k); await page.waitForTimeout(55); }
  ...
  await page.screenshot({ path: `${shotDir}/${String(shot++).padStart(2, '0')}-seed${p.seed}-${p.piece}.png` });
  await page.close();
}
await browser.close();
...
for (const r of results) {
  if (r.keptB2b) pass++;
  console.log(`${r.keptB2b ? 'OK' : '!!'} seed ${r.seed} (${r.piece}) b2b="${r.b2b}" chip="${r.chip}"\n     dock="${r.dock}"`);
}
