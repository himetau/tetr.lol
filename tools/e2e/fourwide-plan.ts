...
interface Step { keys: string[]; desc: string; expect?: string; combo?: number }
...
for (let s = 1; s < 60; s++) {
  ...
  const adv = fourwideAdvice(g.board, [g.active!.type, ...g.preview()] as PieceType[], null, g.pieceIndex);
  if (adv.sustainable && adv.moves.length > 0 && !adv.moves[0].usesHold) { seed = s; break; }
}
if (!seed) throw new Error('no sustainable seed found');

const steps: Step[] = [];
const sim = new Game();
sim.reset(buildFourwideStart(seed).board, seed);

const doPlacement = (piece: PieceType, cells: Cells, desc: string, expect?: string, combo?: number) => {
  let useHold = false;
  if (sim.active!.type !== piece) {
    useHold = true;
    sim.holdPiece();
    if (sim.active!.type !== piece) throw new Error(`sim desync at "${desc}": want ${piece}, active ${sim.active!.type}`);
  }
...
while (placed < 7 && guard++ < 24) {
  ...
  if (!doomShown && adv.moves[0].score >= queue.length) {
    const doom = adv.moves.find((m) => !m.usesHold && m.piece === sim.active!.type && m.score < queue.length);
    if (doom) {
      doomShown = true;
      const p = findPlacement(sim.board, doom.piece, doom.cells as Cells)!;
      steps.push({ keys: keysFor(sim.board, p, false), desc: `doom: ${doom.piece} clears but loses the combo in ${doom.score}`, expect: 'Combo will be lost' });
  ...
  if (!deviated && placed >= 2) {
    ...
    const stray = enumeratePlacements(sim.board, sim.active!.type)
      ...
      .sort((a, b) => b.y - a.y)[0];
    if (stray) {
      steps.push({ keys: keysFor(sim.board, stray, false), desc: `deviation: ${stray.type} stacked without clearing`, expect: 'Combo breaker' });
      steps.push({ keys: ['Escape', 'Ctrl+KeyZ'], desc: 'dismiss + undo the combo breaker', combo });
      // sim never applies it: the app's undo restores this exact state
    }
  }
  combo++;
  doPlacement(mv.piece, mv.cells as Cells, `book ${mv.piece}${mv.usesHold ? ' (hold)' : ''} #${placed}`, 'Book · 4-wide', combo);
  placed++;
}
if (placed < 3) throw new Error(`plan too short: ${placed} placements`);
