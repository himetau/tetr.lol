...
import { WELL_X, WELL_W, WALL_H, wallMask, residualKey, stateToBoard } from '../src/engine/fourwide-core';
...
function parseState(s: string): number {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let col = 0; col < WELL_W; col++) {
      if (lines[i][col] === '1') key |= 1 << (row * WELL_W + col);
    }
  }
  return key;
}
...
if (keyToIndex.size !== stateKeys.length) throw new Error('duplicate canonical state');
...
const states = stateKeys.map((key, idx) => {
  ...
  for (const piece of PIECE_TYPES) {
    ...
    for (const p of enumeratePlacements(board, piece)) {
      ...
      if (p.linesCleared !== 1) throw new Error(`state ${idx} ${piece}: cleared ${p.linesCleared}`);
      const nextKey = residualKey(p.after);
      if (nextKey === null) continue;
      const next = keyToIndex.get(nextKey);
      if (next === undefined) continue; // clears but leaves a non-canonical residual
      cont.push({
        piece,
        rot: p.rot,
        x: p.x,
    ...
    if (cont.length > 0) placements[piece] = cont;
  }
...
for (const [i, s] of states.entries()) {
  ...
  if (pieces.length === 0) throw new Error(`state ${i} has no continuations`);
}
...
if (residualKey(b) !== stateKeys[0]) throw new Error('residualKey round-trip failed');
if ((b.rows[0] & wallMask()) !== wallMask()) throw new Error('walls missing');
