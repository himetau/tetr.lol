...
export class Board {
  ...
  filled(x: number, y: number): boolean {
    if (x < 0 || x >= BOARD_W || y < 0) return true; // walls/floor count as filled
    if (y >= BOARD_H) return false;
    return (this.rows[y] >>> x & 1) === 1;
  }

  collides(cells: Cell[]): boolean {
    for (const [x, y] of cells) if (this.filled(x, y)) return true;
    return false;
  }

  place(cells: Cell[]): void {
    for (const [x, y] of cells) if (y >= 0 && y < BOARD_H) this.rows[y] |= 1 << x;
  }
  ...
  clearLines(): number[] {
    ...
    while (dst < BOARD_H) this.rows[dst++] = 0;
    return cleared;
  }
  ...
  insertGarbage(holes: number[]): void {
    ...
    if (n === 0) return;
    for (let y = BOARD_H - 1; y >= n; y--) this.rows[y] = this.rows[y - n];
  ...
  columnHeight(x: number): number {
    for (let y = BOARD_H - 1; y >= 0; y--) if (this.rows[y] >>> x & 1) return y + 1;
    return 0;
  }

  isEmpty(): boolean {
    for (let y = 0; y < BOARD_H; y++) if (this.rows[y] !== 0) return false;
    return true;
  }
  ...
  toStrings(height?: number): string[] {
    ...
    for (let y = h - 1; y >= 0; y--) {
      let s = '';
      for (let x = 0; x < BOARD_W; x++) s += (this.rows[y] >>> x & 1) ? 'X' : '_';
      out.push(s);
    }
    return out;
  }
