...
export function keyDescriptor(e: KeyboardEvent): string {
  const c = e.code;
  if (/^(Control|Alt|Meta|Shift)/.test(c)) {
    return c;
  }
  let d = "";
  if (e.ctrlKey) {
...
export class InputHandler {
  ...
  keyUp(code: string, _time: number): void {
    ...
    if (action === "left" || action === "right") {
      ...
      if (wasActive && this.dirStack.length > 0) {
        this.changeDirection();
      }
  ...
  private step(dir: Dir): void {
    ...
    if (this.softDropHeld && this.settings.sdf >= 41) {
      this.game.softDropToFloor();
    }
  }
  ...
  private slideToWall(dir: Dir): void {
    let moved = true;
    while (moved) {
      moved = dir === -1 ? this.game.moveLeft() : this.game.moveRight();
    }
    if (this.softDropHeld && this.settings.sdf >= 41) {
      this.game.softDropToFloor();
    }
