...
export class Game {
  ...
  private refillQueue(): void {
    while (this.queue.length < PREVIEW_N + 1) {
  ...
  private tryShift(dx: number, dy: number): boolean {
    ...
    if (this.board.collides(cellsAt(a.type, a.rot, a.x + dx, a.y + dy))) {
      return false;
    }
    a.x += dx;
    a.y += dy;
  ...
  moveLeft(): boolean {
    return this.tryShift(-1, 0);
  }
  moveRight(): boolean {
    return this.tryShift(1, 0);
  }
  softDropStep(): boolean {
    return this.tryShift(0, -1);
  }
  ...
  ghostY(): number {
    ...
    let y = a.y;
    while (!this.board.collides(cellsAt(a.type, a.rot, a.x, y - 1))) {
      y--;
    }
    return y;
  }
  ...
  applyMove(piece: PieceType, cells: [number, number][], spin: SpinKind): LockEvent | null {
    ...
    if (a.type !== piece) {
      if (!this.canHold || !this.holdPiece()) {
        return null;
      }
      a = this.active;
      if (!a || a.type !== piece) {
    ...
    for (let rot = 0 as Rot; rot < 4; rot = (rot + 1) as Rot) {
      ...
      for (const [tx, ty] of cells) {
        for (const [bx, by] of base) {
          const ox = tx - bx;
          const oy = ty - by;
          const got = cellsAt(a.type, rot, ox, oy)
            .map(([x, y]) => x * 64 + y)
            .sort((p, q) => p - q);
  ...
  private lock(spinOverride?: SpinKind): LockEvent | null {
    ...
    const spin =
      spinOverride ??
      detectSpin(this.board, a.type, a.rot, a.x, a.y, this.lastMoveWasRotation, this.lastKickIndex);
    this.board.place(cells);
    for (const [cx, cy] of cells) {
      if (cy >= 0 && cy < BOARD_H) {
        this.colors[cy][cx] = a.type;
      }
    }
    const cleared = this.board.clearLines();
    for (let i = cleared.length - 1; i >= 0; i--) {
      this.colors.splice(cleared[i], 1);
    }
    while (this.colors.length < BOARD_H) {
      this.colors.push(new Array<PieceType | null>(BOARD_W).fill(null));
    }
  ...
  garbageBoardView(): { heights: number[]; garbageAnchor: number } {
    ...
    for (let x = 0; x < BOARD_W; x++) {
      heights.push(this.board.columnHeight(x));
    }
    let garbageAnchor = -1;
    for (let y = this.board.maxHeight() - 1; y >= 0; y--) {
      ...
      let isGarbage = false;
      for (let x = 0; x < BOARD_W; x++) {
        if (((r >>> x) & 1) === 1 && this.colors[y][x] === null) {
          isGarbage = true;
          break;
        }
      }
  ...
  addGarbage(holes: number[]): void {
    ...
    for (let i = 0; i < n; i++) {
      this.colors.unshift(new Array<PieceType | null>(BOARD_W).fill(null));
    }
    if (buried) {
      this.topOut = true;
      this.active = null;
