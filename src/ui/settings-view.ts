...
export function settingsView(): HTMLElement {
  ...
  handling.appendChild(
    sliderRow("DAS", "delayed auto shift (ms)", 0, 300, 1, settings.handling.dasMs, (v) => {
  ...
  handling.appendChild(
    sliderRow(
      "ARR",
      "auto repeat rate (ms) - 0 = instant",
      0,
      80,
      1,
      settings.handling.arrMs,
      (v) => {
  ...
  handling.appendChild(
    sliderRow("SDF", "soft drop factor - 41 = instant", 5, 41, 1, settings.handling.sdf, (v) => {
  ...
  handling.appendChild(
    sliderRow(
      "DCD",
      "DAS cut delay (ms) - pauses DAS after a rotate/hold/drop; 0 = off",
      0,
      100,
      1,
      settings.handling.dcdMs,
      (v) => {
  ...
  handling.appendChild(
    toggleRow(
      "DAS carry across direction change",
      "on = the charge is preserved, so a charged flick bounces wall-to-wall (tetr.io style); off = DAS re-charges",
      settings.handling.dasCarry,
      (v) => {
  ...
  trainer.appendChild(
    selectRow(
      "Feedback",
      "which placements get a grade chip",
      ...
      (v) => {
        settings.feedbackLevel = v as typeof settings.feedbackLevel;
      },
    ),
  );
  trainer.appendChild(
    toggleRow(
      "Stop on mistake",
      "pause and open alternatives when you misplace",
      settings.stopOnMistake,
      (v) => {
  ...
  trainer.appendChild(
    toggleRow(
      "Neural evaluator",
      "learned correction on top of the heuristic grading",
      settings.neuralEval,
      (v) => {
  ...
  trainer.appendChild(
    toggleRow(
      "Auto-retry on top out",
      "start a fresh drill automatically",
      settings.autoRetryTopOut,
      (v) => {
  ...
  for (const m of Object.keys(EVAL_LABEL) as GradedMode[]) {
    ...
    trainer.appendChild(
      toggleRow(
        name,
        hint + " - off hides grades, chips and the paths panel",
  ...
  vs.appendChild(
    selectRow(
      "Bot strength",
      "Cold Clear search budget per move",
      v.botLevel,
      [
        ["easy", "easy"],
        ["normal", "normal"],
  ...
  vs.appendChild(
    sliderRow(
      "Custom nodes",
      'search nodes per move when strength is "custom"',
      500,
      100000,
      500,
      v.botNodes,
  ...
  vs.appendChild(
    sliderRow("Bot speed", "pieces per second", 0.5, 4, 0.25, v.botPps, (val) => {
      v.botPps = val;
    }),
  );
  vs.appendChild(
  ...
  vs.appendChild(
    sliderRow(
      "Bot attack",
      "scales the bot’s outgoing garbage (%) - a handicap dial",
      25,
      300,
      25,
      v.botAttackScale,
  ...
  vs.appendChild(
    sliderRow(
      "Garbage delay",
      "telegraph time (ms) before an attack can rise",
      500,
      5000,
      250,
      v.garbageDelayMs,
  ...
  vs.appendChild(
    sliderRow(
      "Messiness",
      "chance (%) each row re-rolls the hole within an attack - new attacks always re-roll (tetr.io: 0)",
      0,
      100,
      5,
      v.messiness,
  ...
  vs.appendChild(
    sliderRow(
      "Garbage cap",
      "max rows rising on one non-clearing lock",
      1,
      12,
      1,
      v.garbageCap,
  ...
  vs.appendChild(
    selectRow(
      "First to",
      "rounds needed to take a 1v1 match",
      String(v.firstTo),
      [
        ["1", "1"],
        ["2", "2"],
  ...
  vs.appendChild(
    sliderRow("Quad attack", "lines a quad sends", 0, 8, 1, v.rules.quadAttack, (val) => {
  ...
  vs.appendChild(
    sliderRow(
      "B2B bonus",
      "extra lines while the back-to-back chain is alive",
      0,
      4,
      1,
      v.rules.b2bBonus,
  ...
  vs.appendChild(
    sliderRow(
      "Combo interval",
      "attack += floor(combo ÷ this); 0 turns combo damage off",
      0,
      4,
      1,
      v.rules.comboDiv,
  ...
  vs.appendChild(
    sliderRow("All clear", "lines a perfect clear adds", 0, 20, 1, v.rules.allClear, (val) => {
  ...
  vs.appendChild(
    selectRow(
      "Simulated pressure",
      'attack pace when a drill uses "garbage" instead of the bot',
      v.pressure,
      [
        ["calm", "calm"],
        ["normal", "normal"],
      ...
      (val) => {
        v.pressure = val as Pressure;
      },
    ),
  );
  const oppOptions: [string, string][] = [
    ["off", "off"],
    ["garbage", "garbage only"],
    ["bot", "cold clear bot"],
  ];
  vs.appendChild(
    selectRow(
      "4-wide opponent",
      "pressure during the 4-wide drill",
      v.drill.fourwide,
      oppOptions,
      (val) => {
  ...
  vs.appendChild(
    selectRow(
      "40 lines opponent",
      "pressure during the sprint - turns it into a dig race",
      v.drill.free,
      oppOptions,
      (val) => {
  ...
  vs.appendChild(
    selectRow(
      "All-Spin opponent",
      "pressure during the all-spin drill",
      v.drill.allspin,
      oppOptions,
      (val) => {
  ...
  sound.appendChild(
    toggleRow("Sound effects", "movement, clears, combos and milestones", settings.soundFx, (v) => {
      settings.soundFx = v;
    }),
  );
  sound.appendChild(
    toggleRow(
      "Mistake sound",
      "audio cue on mistakes and blunders",
      settings.soundOnMistake,
      (v) => {
  ...
  sound.appendChild(
    volumeRow("Master volume", "scales every sound", "master", () =>
      sfx("clearline", 0.5, "master"),
    ),
  );
  sound.appendChild(
    volumeRow("Movement", "move, rotate, drop, hold", "move", () => sfx("harddrop", 0.5, "move")),
  );
  sound.appendChild(
    volumeRow("Clears & combos", "line clears, spins, B2B and combo jingles", "clear", () =>
      sfx("clearquad", 0.55, "clear"),
    ),
  );
  sound.appendChild(
    volumeRow("Alerts & events", "garbage, danger, countdowns, mistakes, milestones", "alert", () =>
      sfx("levelup", 0.55, "alert"),
    ),
  );
  ...
  theme.appendChild(
    selectRow(
      "Preset",
      "a built-in palette, or your own colours",
  ...
  appearance.appendChild(
    sliderRow("Board zoom", "field size (%)", 60, 160, 5, settings.boardZoom, (v) => {
      settings.boardZoom = v;
    }),
  );
  appearance.appendChild(
    toggleRow("Ghost piece", "", settings.ghost, (v) => {
      settings.ghost = v;
    }),
  );
  appearance.appendChild(
    toggleRow("Grid", "", settings.grid, (v) => {
      settings.grid = v;
    }),
  );
  appearance.appendChild(
    toggleRow("Board effects", "particles, screen shake and action text", settings.effects, (v) => {
      settings.effects = v;
    }),
  );
  appearance.appendChild(
    toggleRow(
      "FPS counter",
      "live frame rate pinned to the right edge",
      settings.fpsCounter,
      (v) => {
  ...
  bg.appendChild(
    selectRow(
      "Backdrop",
      "rotates behind the app, like tetr.io",
      ...
      (v) => {
        settings.background.mode = v as typeof settings.background.mode;
      },
    ),
  );
  bg.appendChild(
    sliderRow(
      "Dim",
      "overlay strength - lower shows more of the image",
      0,
      95,
      5,
  ...
  bg.appendChild(
    sliderRow(
      "Cycle",
      "seconds between background changes",
      15,
      600,
      15,
      settings.background.cycleSec,
      (v) => {
...
function bgImagesRow(): HTMLElement {
  const r = row(
    "My images",
    "stored in this browser/app only - adding switches the backdrop to “my images”",
  );
  ...
  const refresh = () => {
    ...
    countEl.textContent =
      n === 0 ? "no images added yet" : `${n} image${n > 1 ? "s" : ""} in the cycle`;
  };
  refresh();
  ...
  r.append(
    filePick,
    folderPick,
    btn("Add images…", () => filePick.click()),
    btn("Add folder…", () => folderPick.click()),
    btn("Next ▸", () => nextBackground()),
    btn("Clear", () => {
