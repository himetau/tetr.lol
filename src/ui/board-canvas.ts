...
function skin(): HTMLImageElement {
  if (!skinImg) {
    ...
    skinImg.onload = () => {
      skinReady = true;
      for (const f of skinWaiters.splice(0)) f();
    };
  }
  return skinImg;
}
...
function pieceNeighbors(cells: readonly (readonly [number, number])[]): (x: number, y: number) => Neighbors {
  const set = new Set(cells.map(([x, y]) => x * 64 + y));
  const has = (x: number, y: number) => set.has(x * 64 + y);
  return (x, y) => ({ up: has(x, y + 1), down: has(x, y - 1), left: has(x - 1, y), right: has(x + 1, y) });
}

// ---- canvas fx primitives ----

interface Particle {
  x: number; y: number;       // px, canvas space
  vx: number; vy: number;     // px/s
  age: number; ttl: number;   // ms
  size: number;
  color: string;
  grav: number;               // px/s²
}

interface RowFlash { y: number; age: number }          // board row, pre-clear
interface CellFlash { cells: readonly (readonly [number, number])[]; age: number }
interface Trail { x0: number; x1: number; yTop: number; age: number; color: string }
...
export class FieldRenderer {
  ...
  fxDrop(cells: readonly (readonly [number, number])[], color: string): void {
    ...
    this.trails.push({ x0: x0 * this.cell, x1: x1 * this.cell, yTop: this.py(yTop - 1), age: 0, color });
    for (const [x, y] of cells) {
  ...
  fxGarbage(rows: number): void {
    ...
    for (let i = 0; i < rows * 10; i++) {
      this.spawn({
        x: Math.random() * w, y: h - Math.random() * rows * this.cell,
        vx: (Math.random() - 0.5) * 160,
        vy: -(60 + Math.random() * 200),
        age: 0, ttl: 300 + Math.random() * 300,
  ...
  fxTopout(board: Board, colors: (PieceType | null)[][] | null): void {
    ...
    for (let y = 0; y < VISIBLE_H; y++) {
      for (let x = 0; x < BOARD_W; x++) {
        ...
        const t = colors?.[y]?.[x];
        this.spawn({
          x: (x + 0.5) * this.cell, y: this.py(y) + this.cell * 0.5,
          vx: (Math.random() - 0.5) * 340,
          vy: -(40 + Math.random() * 320),
          age: 0, ttl: 500 + Math.random() * 600,
  ...
  private spawn(p: Particle): void {
    if (this.particles.length >= P_MAX) this.particles.shift();
...
export function renderPieceTile(type: PieceType | null, cell = 18): HTMLCanvasElement {
  ...
  paint();
  if (!skinReady) whenSkinReady(paint);
  return canvas;
}
