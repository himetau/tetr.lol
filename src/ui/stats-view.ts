...
export function statsView(): HTMLElement {
  ...
  const chartCard = (
    ...
    if (points.length < 2) {
      ...
      c.appendChild(
        lineChart(
          { color, points: points.slice(-TREND_WINDOW).map((p, i) => ({ ...p, x: i })) },
          {
  ...
  if (lst.length > 0) {
    chartCard(
      "LST drill - accuracy per session",
      SERIES_VAR.lst,
      lst.map((s) => ({
        x: 0,
  ...
  if (fw.length > 0) {
    ...
    chartCard(
      "4-wide - longest combo per session",
      SERIES_VAR.fourwide,
      fw.map((s) => ({
        x: 0,
        y: s.maxCombo!,
  ...
  if (asB2b.length >= 2 || (asB2b.length > 0 && asAcc.length < 2)) {
    ...
    chartCard(
      "All-Spin - best B2B chain per session",
      SERIES_VAR.allspin,
      asB2b.map((s) => ({
        x: 0,
        y: s.maxB2b!,
  ...
  if (sprints.length > 0) {
    ...
    chartCard(
      ...
      quarterTicks(slowest, 60, 20),
      (v) => fmtSprint(v * 1000, false),
  ...
  if (stats.sessions.length === 0) {
    ...
    empty.appendChild(
      emptyNote(
        "finish a couple of ranked sessions (5+ placements, no undo/bot - retry, top out, or leave the drill records it) and per-mode trends appear here",
      ),
    );
  ...
  const played = (["lst", "fourwide", "free", "quick", "allspin", "versus"] as Mode[]).filter(
    (m) => stats.modes[m].pieces > 0 || stats.modes[m].drills > 0,
  );
  if (played.length === 0) {
    ...
    for (const m of played) {
      ...
      const avgPps =
        ppsVals.length > 0 ? (ppsVals.reduce((a, b) => a + b, 0) / ppsVals.length).toFixed(2) : "-";
      const acc = gradeTotal(s.grades) > 0 ? `${(accuracy(s) * 100).toFixed(1)}%` : "-";
      const tr = document.createElement("tr");
      tr.innerHTML =
        `<td>${modeDot(m)}${MODE_LABEL[m]}</td><td>${s.drills}</td><td>${s.pieces}</td><td>${s.tsds}</td>` +
  ...
  if (recent.length === 0) {
    ...
    for (const s of recent) {
      ...
      const pps = sessionPps(s);
      tr.innerHTML =
        `<td>${fmtDate(s.at)}</td><td>${modeDot(s.mode)}${SHORT_LABEL[s.mode]}</td><td>${s.pieces}</td>` +
...
function modeRecord(m: Mode, sessions: SessionRecord[]): string {
  switch (m) {
    ...
    case "lst": {
      const accs = sessions
        .filter((s) => gradeTotal(s.grades) > 0)
        .map((s) => gradeAccuracy(s.grades));
...
function sessionResult(s: SessionRecord): string {
  ...
  switch (s.mode) {
    case "quick":
      return `${Math.round(s.altitude ?? 0)}m`;
    case "versus":
      return `${(s.wins ?? 0) > (s.losses ?? 0) ? "won" : "lost"} ${s.wins ?? 0}–${s.losses ?? 0}`;
    case "fourwide":
      return s.maxCombo !== undefined
        ? `combo ×${s.maxCombo}${acc ? ` · ${acc}` : ""}`
        : acc || "-";
    case "free":
      return s.sprintMs !== undefined
        ? `${fmtSprint(s.sprintMs)}${acc ? ` · ${acc}` : ""}`
        : "unfinished";
    case "allspin":
      return s.maxB2b !== undefined ? `B2B ×${s.maxB2b}${acc ? ` · ${acc}` : ""}` : acc || "-";
    case "lst":
...
function lineChart(
  series: ChartSeries,
  opts: { slots: number; yTicks: number[]; yFmt: (v: number) => string },
): HTMLElement {
  ...
  const px = (slot: number) =>
    PAD.l + (opts.slots === 1 ? plotW / 2 : (slot / (opts.slots - 1)) * plotW);
