...
export function sfx(name: SfxName, volume = 0.5, cat: SfxCategory = "alert", rate = 1): void {
  try {
    ...
    // the inverted comparison also guards NaN volumes
    if (!(level > 0)) {
      return;
    }
...
export function actionSound(action: string): void {
  switch (action) {
    ...
    case "rotate180":
      ...
      break;
    case "softDrop":
      sfx("softdrop", 0.35, "move");
      break;
    case "hardDrop":
      sfx("harddrop", 0.5, "move");
      break;
    case "hold":
      sfx("hold", 0.45, "move");
      break;
...
export function clearSound(lines: number, tspin: boolean, b2bChain = 0, allClear = false): void {
  ...
  if (tspin) {
    sfx(b2bChain > 1 ? "clearbtb" : "clearspin", 0.55, "clear");
  } else if (lines >= 4) {
    sfx(b2bChain > 1 ? "clearbtb" : "clearquad", 0.55, "clear");
  } else {
    sfx("clearline", 0.5, "clear");
  }
...
export function b2bCue(chain: number): { name: SfxName; rate: number } | null {
  // back-to-back begins at the second consecutive special
  if (chain < 2) {
    return null;
  }
  ...
  if (n <= 3) {
    return { name: `btb_${n}` as SfxName, rate: 1 };
  }
...
export function b2bSound(chain: number): void {
  ...
  if (cue) {
    sfx(cue.name, 0.5, "clear", cue.rate);
  }
}
