...
import { invalidateCssVars } from "./css";
...
export function activePalette(): Palette {
  ...
  if (p.preset === "custom") {
    return { ...THEMES["rose-pine"], ...p.custom };
  }
...
export function applyTheme(): void {
  ...
  const pal =
    p.preset === "custom"
      ? { ...THEMES["rose-pine"], ...p.custom }
      : (THEMES[p.preset] ?? THEMES["rose-pine"]);
  for (const [v] of THEME_VARS) {
    if (pal[v]) {
      root.style.setProperty("--" + v, pal[v]);
    }
  }
  for (const [v, expr] of DERIVED) {
    root.style.setProperty("--" + v, expr);
  }
  // text on the accent colour: contrast against the accent's brightness
  root.style.setProperty(
    "--on-accent",
    luminance(pal.accent ?? "#000000") > 0.55 ? "#15141c" : "#ffffff",
  );
  invalidateCssVars();
}
