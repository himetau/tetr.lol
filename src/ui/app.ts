...
const ICONS: Record<ViewName | "sun" | "moon", string> = {
  ...
  lst: fico(
    '<rect x="2.6" y="4.6" width="6" height="6"/><rect x="9" y="4.6" width="6" height="6"/><rect x="15.4" y="4.6" width="6" height="6"/><rect x="9" y="11" width="6" height="6"/>',
  ),
  ...
  patterns: fico(
    '<rect x="3" y="6.6" width="13" height="13" opacity="0.5"/><rect x="8" y="4" width="13" height="13"/>',
  ),
  // stopwatch - sprint against the clock (square clock body)
  free: fico(
    '<rect x="9.4" y="1.5" width="5.2" height="2.7"/><rect x="16.9" y="3.4" width="2.3" height="3.6" transform="rotate(42 18.05 5.2)"/><path fill-rule="evenodd" d="M3.5 5h17v17h-17zm3 3v11h11V8z"/><rect x="11.2" y="8.7" width="1.7" height="5.9" transform="rotate(37 12 13.5)"/><rect x="10.85" y="12.35" width="2.3" height="2.3"/>',
  ),
  // mountain summit - climb to the zenith (back peak dimmed for depth, square sun)
  quick: fico(
    '<path opacity="0.45" d="M8.5 20.5 14.9 9l6.4 11.5z"/><path d="M1.7 20.5 9.4 7l7.7 13.5z"/><rect x="7.32" y="3" width="4.2" height="4.2"/>',
  ),
  ...
  versus: fico(
    '<g transform="translate(12 12) scale(1.15)"><g transform="rotate(-45)" opacity="0.5"><path d="M-1.7 0.5H1.7V-7H-1.7Z M-1 -7H1V-9.4H-1Z M-3 0.5H3V2.4H-3Z M-1.2 2.4H1.2V6.6H-1.2Z"/></g><g transform="rotate(45)"><path d="M-1.7 0.5H1.7V-7H-1.7Z M-1 -7H1V-9.4H-1Z M-3 0.5H3V2.4H-3Z M-1.2 2.4H1.2V6.6H-1.2Z"/></g></g>',
  ),
  // ascending bars - your progress climbing (sharp bars)
  stats: fico(
    '<rect x="2.8" y="12" width="4.4" height="8.6"/><rect x="9.8" y="7.8" width="4.4" height="12.8"/><rect x="16.8" y="3.6" width="4.4" height="17"/>',
  ),
  // gear - handling / keys (square cog with square teeth)
  settings: fico(
    '<path fill-rule="evenodd" d="M10 4h4v2h4v4h2v4h-2v4h-4v2h-4v-2H6v-4H4v-4h2V6h4zM9.5 9.5h5v5h-5z"/>',
  ),
  moon: sico('<path d="M14 3h-2v2h-2v2H8v2H6v6h2v2h2v2h2v2h2v-2h-2v-2h-2v-2H8V9h2V7h2V5h2z"/>'),
  sun: sico(
    '<rect x="8" y="8" width="8" height="8"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/>',
  ),
};
...
export function startApp(root: HTMLElement): void {
  ...
  const show = (name: ViewName) => {
    ...
    if (name === "lst" || name === "fourwide" || name === "free" || name === "allspin") {
      ...
      if (name === "patterns") {
        mountLazy(name, () => import("./patterns-view").then((m) => m.patternsView()));
      } else if (name === "stats") {
        mountLazy(name, () => import("./stats-view").then((m) => m.statsView()));
      } else {
        mountLazy(name, () => import("./settings-view").then((m) => m.settingsView()));
      }
      return;
    }
  ...
  for (const item of NAV) {
    ...
    b.addEventListener("click", () => {
      if (b !== activeBtn && settings.soundFx) {
        sfx("move", 0.2, "master");
      }
      show(item.name);
    });
