...
export function enumeratePlacements(board: Board, type: PieceType): Placement[] {
  if (collidesFast(board, type, 0, SPAWN_X, SPAWN_Y)) return [];
  ...
  while (qh < queue.length) {
    ...
    for (const move of MOVES) {
      let nx = n.x, ny = n.y, nrot = n.rot, nkick = -1;
      if (move === 'left' || move === 'right') {
        nx += move === 'left' ? -1 : 1;
        if (collidesFast(board, type, nrot, nx, ny)) continue;
      } else if (move === 'sd') {
        ny -= 1;
        if (collidesFast(board, type, nrot, nx, ny)) continue;
      } else {
        ...
        if (!res) continue;
        nx = res.x; ny = res.y; nrot = res.rot; nkick = res.kickIndex;
      }
  ...
  for (const { node, spin } of results.values()) {
    const path: string[] = [];
    for (let p: Node | null = node; p && p.move; p = p.parent) path.unshift(p.move);
  ...
  if (type === 'T') {
    ...
    for (let y = 0; y <= maxY; y++) {
      for (let x = 1; x < BOARD_W - 1; x++) {
        ...
        if (!collidesFast(board, 'T', 2, x, y - 1)) continue; // not grounded
        if (!board.filled(x - 1, y + 1) && !board.filled(x + 1, y + 1)) continue; // no roof
        const spin = detectSpin(board, 'T', 2, x, y, true, 0);
        if (spin === 'none') continue;
