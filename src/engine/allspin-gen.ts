...
function carve(piece: PieceType, rot: Rot, x: number, y: number, overhang: [number, number][]): Board | null {
  const cells = cellsAt(piece, rot, x, y);
  for (const [cx, cy] of cells) if (cx < 0 || cx >= BOARD_W || cy < 0) return null;
  const ys = [...new Set(cells.map(([, cy]) => cy))].sort((a, b) => a - b);
  const board = new Board();
  const isPiece = (cx: number, cy: number) => cells.some(([a, b]) => a === cx && b === cy);
  for (const ry of ys) {
    board.rows[ry] = FULL;
    for (const [cx, cy] of cells) if (cy === ry) board.rows[ry] &= ~(1 << cx);
  }
  for (const [ox, oy] of overhang) {
    if (ox < 0 || ox >= BOARD_W || oy < 0 || isPiece(ox, oy)) return null;
    board.rows[oy] |= 1 << ox;
  }
  // no row may already be complete (that would clear on its own)
  for (const ry of ys) if (board.rows[ry] === FULL) return null;
  return board;
}
...
function settle(board: Board): Board {
  let shift = 0;
  while (shift < 26 && board.rows[shift] === 0 && board.rows.some((r, i) => i > shift && r !== 0)) shift++;
  if (shift === 0) return board;
  const b = new Board();
  for (let y = 0; y + shift < 26; y++) b.rows[y] = board.rows[y + shift];
  return b;
}
  ...
  for (let preRot = 0 as Rot; preRot < 4; preRot = (preRot + 1) as Rot) {
    for (let preX = 0; preX < BOARD_W; preX++) {
      for (const dir of [1, -1, 2] as const) {
        ...
        const ev = g.hardDrop();
        if (ev && ev.spin !== 'none' && ev.linesCleared >= 2) return true;
      }
    }
  }
  return false;
...
export interface AllspinSetup { board: Board; spinPiece: PieceType }
...
export function genAllspin(seed: number, garbage = false): AllspinSetup {
  ...
  for (let attempt = 0; attempt < 400; attempt++) {
    ...
    const overhang: [number, number][] = [[cx + ohSide, 2], ...(rng() < 0.4 ? [[cx + ohSide, 3] as [number, number]] : [])];
    let board = carve(piece, rot, cx, 1, overhang);
    if (!board) continue;
    board = settle(board);
    if (garbage) board = addGarbage(board, rng);
    if (board.maxHeight() > 12) continue;
    if (hasSpinDouble(board, piece)) return { board, spinPiece: piece };
  }
...
function addGarbage(board: Board, rng: () => number): Board {
  ...
  const b = new Board();
  for (let y = 0; y + rows < 26; y++) b.rows[y + rows] = board.rows[y];
  for (let i = 0; i < rows; i++) {
    let row = FULL;
    const holes = 1 + Math.floor(rng() * 2);
    for (let h = 0; h < holes; h++) row &= ~(1 << Math.floor(rng() * BOARD_W));
    b.rows[i] = row;
  }
  return b;
}
