...
function legal(p: Placement): boolean {
  ...
  if (p.type === "I" && p.linesCleared > 0) {
    return false;
  }
  if (p.linesCleared > 0 && p.linesCleared < 4 && p.spin === "none") {
    return false;
  }
  if (!isTsd(p) && !findLstSite(p.after)) {
    return false;
  }
  return true;
}
...
export function lstLoopMove(
  ...
  for (let d = 0; d < horizon; d++) {
    ...
    const scored = next.map((n) => ({ n, score: scoreNode(n) })).sort((a, b) => b.score - a.score);
...
function scoreNode(n: Node): number {
  ...
  return (
    n.tsds * 100000 + ev - n.board.maxHeight() * 60 - cells * 6 - (site ? site.missing * 30 : 5000)
  );
}
