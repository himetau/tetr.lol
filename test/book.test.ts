...
describe('lst cover book', () => {
  ...
  it('suggests each placement while following a bag-2 solution', () => {
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      expect(matchesBookMove(adv, next.piece as PieceType, next.cells as [number, number][])).toBe(true);
  ...
  it('never calls a queue unsustainable that sfinder covers (bag 1)', () => {
    const csv = readFileSync(join(__dirname, '..', 'tools', 'data', 'cover', 'flattop LST bag 1 cover.csv'), 'utf8')
      .trim()
      .split('\n');
  ...
  it('chains from a four.lol TKI opener into the bag-2 book', () => {
    ...
    for (let y = 0; y < 26; y++) {
      for (let x = 0; x < 10; x++) if (startBoard.rows[y] >>> x & 1) mirrored.rows[y] |= 1 << (9 - x);
    }
  ...
  it('grades a book move as best and a deviation with a book hint', () => {
    ...
    const board = boardFrom(bag2Group.start);
    const key = (cs: [number, number][]) => cs.map(([x, y]) => x * 32 + y).sort((a, b) => a - b).join(',');
    ...
    for (const section of (lstPatterns as any).lst) {
      for (const pat of section.patterns) {
        for (const page of pat.pages) {
          ...
          if (!rows.length || rows.length > 7) continue;
          const board = boardFrom(rows);
          for (const p of enumeratePlacements(board, 'T')) {
            if (p.spin !== 'full' || p.linesCleared < 2 || !findLstSite(p.after)) continue;
            const r = gradePlacement({
              rows: Array.from(board.rows), queue, hold: null,
              userCells: p.cells as [number, number][], userPiece: 'T',
              userRot: p.rot, userX: p.x, userY: p.y, userSpin: p.spin, userLines: p.linesCleared,
              usedHold: false, pieceIndex: 14, lstBias: true,
            });
            checked++;
            expect(['best', 'good'], `${section.heading}: TSD graded ${r.grade} (${r.reasons.join('; ')})`).toContain(r.grade);
          }
        }
      }
    }
