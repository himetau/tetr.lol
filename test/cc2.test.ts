...
describe("Cold Clear 2 (all-spin) bot", () => {
  beforeAll(async () => {
    const wasmPath = fileURLToPath(
      new URL("../src/engine/cc2/cold_clear_2_bg.wasm", import.meta.url),
    );
  ...
  it("recognizes non-T all-spins (the movegen patch)", () => {
    ...
    for (const [piece, rows, queue] of boards) {
      ...
      if (m!.lines >= 2) {
        expect(m!.spin, `${piece} double should be a spin`).toBe("f");
      }
    }
  });
  ...
  it("never breaks an active back-to-back on a spin-able board", () => {
    ...
    for (let seed = 1; seed <= 12; seed++) {
      ...
      expect(
        keepsB2b(m),
        `seed ${seed}: bot broke B2B with ${m.piece} ${m.lines} lines spin=${m.spin}`,
      ).toBe(true);
    }
  });
