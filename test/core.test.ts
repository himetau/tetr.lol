...
describe("pieces", () => {
  ...
  it("I east state is the vertical column at x=+1", () => {
    expect(PIECE_CELLS.I[1]).toEqual([
      [1, 1],
      [1, 0],
      [1, -1],
      [1, -2],
...
describe("board", () => {
  it("clears full lines and shifts down", () => {
    const b = Board.fromStrings(["X_________", "XXXXXXXXXX", "XXXXXXXXXX", "X_XXXXXXXX"]);
...
describe("T-spin detection", () => {
  it("detects a full TSD in a standard slot", () => {
    ...
    const b = Board.fromStrings(["XX_XXXXXXX", "X___XXXXXX", "XX_XXXXXXX"]);
