...
import { versusAttack, scaleAttack, GarbageQueue, ScheduledAttacker, DEFAULT_GARBAGE, DEFAULT_RULES, type GarbageConfig } from '../src/core/versus';
...
describe('garbage queue', () => {
  ...
  it('holes stay inside the configured column range (4-wide well)', () => {
    const q = new GarbageQueue(cfg({ delayMs: 0, messiness: 1, holeMin: 3, holeMax: 6 }), mulberry32(4));
    q.queue(30, 0);
...
describe('scheduled attacker', () => {
  it('fires attacks over time, sized within the pressure cap', () => {
    ...
    const fired: number[] = [];
    for (let t = 0; t < 120000; t += 100) fired.push(...s.tick(100));
  ...
  it('calm pressure fires far less often than brutal', () => {
    const count = (p: 'calm' | 'brutal') => {
      ...
      let n = 0;
      for (let t = 0; t < 300000; t += 100) n += s.tick(100).length;
      return n;
    };
